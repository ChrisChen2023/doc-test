...
<ProductName />  提供了基于 Docker 的快速部署方式，可以在几分钟内完成部署。

=== "使用 Docker Desktop"    

    按照以下步骤可以快速在 Docker Desktop 中部署 <ProductName /> 。
...
2. 在仪表盘中单击`Extensions`或`Add Extensions`打开Extensions Marketplace 搜索 <ProductName /> ，也可以点击 [<ProductName /> ](https://hub.docker.com/extensions/weygu/nebulagraph-dd-ext) 在 Docker Desktop 打开。
   
3. 单击 **Install** 下载 <ProductName /> 。
