...
本文以一个示例说明如何使用 Exchange 将存储在 Neo4j 的数据导入 <ProductName_CN /> 。
...
至此，Exchange 即完成了对 Neo4j 数据的导出。之后，数据被并行写入 <ProductName_CN /> 数据库中。

整个过程如下图所示。

![NebulaGraph&reg; Exchange 从 Neo4j 数据库中导出数据再并行导入 <ProductName_CN /> 数据库中](https://docs-cdn.nebula-graph.com.cn/figures/ex-ug-002.png "NebulaGraph&reg; Exchange 迁移 Neo4j 数据")
...
#### tags.vertex 或 edges.vertex 配置说明
 ...
 <ProductName_CN /> 在创建点和边时会将 ID 作为唯一主键，如果主键已存在则会覆盖该主键中的数据。所以，假如将某个 Neo4j 属性值作为 <ProductName_CN /> 的 ID，而这个属性值在 Neo4j 中是有重复的，就会导致重复 ID，它们对应的数据有且只有一条会存入 <ProductName_CN /> 中，其它的则会被覆盖掉。由于数据导入过程是并发地往 <ProductName_CN /> 中写数据，最终保存的数据并不能保证是 Neo4j 中最新的数据。
...
运行如下命令将文件数据导入到 <ProductName_CN /> 中。关于参数的说明，请参见[导入命令参数](../parameter-reference/ex-ug-para-import-command.md)。
