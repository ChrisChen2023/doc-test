...
本文简单说明如何使用 Exchange 将存储在 Pulsar 上的数据导入 <ProductName_CN /> 。
  ...
  tags: [
    # 设置 Tag player 相关信息。
    {
      ...
      type: {
        ...
        source: pulsar
        # 指定如何将数据导入 <ProductName_CN /> 。只支持 Client。
        sink: client
      }
      # Pulsar 服务器地址。
...
运行如下命令将 Pulsar 数据导入到 <ProductName_CN /> 中。关于参数的说明，请参见[导入命令参数](../parameter-reference/ex-ug-para-import-command.md)。
