...
本文以一个示例说明如何使用 Exchange 将存储在 HDFS 或本地的 JSON 文件数据导入 <ProductName_CN /> 。
...
```conf
{
  ...
  tags: [
    # 设置 Tag player 相关信息。
    {
      ...
      # 在 fields 里指定 JSON 文件中 key 名称，其对应的 value 会作为 <ProductName_CN /> 中指定属性的数据源。
      # 如果需要指定多个值，用英文逗号（,）隔开。
      fields: [age,name]
...
运行如下命令将 JSON 文件数据导入到 <ProductName_CN /> 中。关于参数的说明，请参见[导入命令参数](../parameter-reference/ex-ug-para-import-command.md)。
