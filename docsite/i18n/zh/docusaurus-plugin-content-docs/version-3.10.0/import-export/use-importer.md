...
NebulaGraph Importer（简称 Importer）是一款 <ProductName /> 的 CSV 文件单机导入工具，可以读取并批量导入多种数据源的 CSV 文件数据，还支持批量更新和删除操作。
...
NebulaGraph Importer 版本和 <ProductName /> 内核的版本对应关系如下。

| <ProductName /> 版本|NebulaGraph Importer 版本|
|:---|:---|
| 3.x.x      |  3.x.x、4.x.x        | 
| 2.x.x      |  2.x.x、3.x.x        |
...
-  <ProductName /> 中已创建 Schema，包括图空间、Tag 和 Edge type，或者通过参数`manager.hooks.before.statements`设置。

## 操作步骤

### 创建 CSV 文件

准备好待导入的 CSV 文件并配置 YAML 文件，即可使用本工具向 <ProductName /> 批量导入数据。
...
NebulaGraph Importer 的 [Github](https://github.com/vesoft-inc/nebula-importer/tree/{{importer.branch}}/examples) 内提供多种示例配置文件。配置文件用来描述待导入文件信息、 <ProductName /> 服务器信息等。下文将分类介绍配置文件内的字段。
...
Client 配置存储客户端连接 <ProductName /> 相关的配置。
...
|参数|默认值|是否必须|说明|
|:---|:---|:---|:---|
|`client.version`|`v3`|是|指定连接的 <ProductName /> 的大版本。当前仅支持`v3`。|
|`client.address`|`"127.0.0.1:9669"`|是|指定连接的 <ProductName /> 地址。多个地址用英文逗号（,）分隔。|
|`client.user`|`root`|否| <ProductName /> 的用户名。|
|`client.password`|`nebula`|否| <ProductName /> 用户名对应的密码。|
