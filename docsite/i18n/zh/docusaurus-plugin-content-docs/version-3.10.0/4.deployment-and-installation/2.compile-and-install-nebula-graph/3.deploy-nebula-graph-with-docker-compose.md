# 使用 Docker Compose 部署 <ProductName_CN /> 

使用 Docker Compose 可以基于准备好的配置文件快速部署 <ProductName_CN /> 服务，仅建议在测试 <ProductName_CN /> 功能时使用该方式。
...
- 如果使用非 root 用户部署 <ProductName_CN />，请授权该用户 Docker 相关的权限。详细信息，请参见 [Manage Docker as a non-root user](https://docs.docker.com/engine/install/linux-postinstall/#manage-docker-as-a-non-root-user)。

- 启动主机上的 Docker 服务。

- 如果已经通过 Docker Compose 在主机上部署了另一个版本的 <ProductName_CN />，为避免兼容性问题，需要删除目录`nebula-docker-compose/data`。
...
3. 执行如下命令启动 <ProductName_CN /> 服务。

  :::note

  - 如果长期未内核更新镜像，请先更新 [<ProductName_CN /> 镜像](#nebulagraph_docker)和 [NebulaGraph Console 镜像](#nebulagraph_console)。
...
连接 <ProductName_CN /> 有两种方式：

- 在容器外通过 Nebula Console 连接。因为容器的配置文件中将 Graph 服务的外部映射端口也固定为 9669，因此可以直接通过默认端口连接。详情参见[连接 <ProductName_CN /> ](../../2.quick-start/3.quick-start-on-premise/3.connect-to-nebula-graph.md)。
...
3. 通过 NebulaGraph Console 连接 <ProductName_CN /> 。
...
4. 查看集群状态。
  ...
  +-------------+------+----------+--------------+----------------------+------------------------+---------+
  | "storaged0" | 9779 | "ONLINE" | 0            | "No valid partition" | "No valid partition"   | "<Var name="nebula_release" />" |
  | "storaged1" | 9779 | "ONLINE" | 0            | "No valid partition" | "No valid partition"   | "<Var name="nebula_release" />" |
  | "storaged2" | 9779 | "ONLINE" | 0            | "No valid partition" | "No valid partition"   | "<Var name="nebula_release" />" |
  +-------------+------+----------+--------------+----------------------+------------------------+---------+
  ```
...
## 查看 <ProductName_CN /> 服务的状态和端口

执行命令`docker-compose ps`可以列出 <ProductName_CN /> 服务的状态和端口。

:::note
 <ProductName_CN /> 默认使用`9669`端口为客户端提供服务，如果需要修改端口，请修改目录`nebula-docker-compose`内的文件`docker-compose.yaml`，然后重启 <ProductName_CN /> 服务。
...
## 查看 <ProductName_CN /> 服务的数据和日志

 <ProductName_CN /> 的所有数据和日志都持久化存储在`nebula-docker-compose/data`和`nebula-docker-compose/logs`目录中。
...
## 重启 <ProductName_CN /> 服务

重启所有 <ProductName_CN /> 服务：
...
## 停止并删除 <ProductName_CN /> 服务

用户可以执行如下命令停止并删除 Docker Compose启动的所有 <ProductName_CN /> 服务：


:::danger

该命令会停止并删除所有 <ProductName_CN /> 服务的容器，以及相关网络。如果用户在`docker-compose.yaml`中定义了卷（`volumes`），则会保留相关数据。
...
### 如何升级/更新 <ProductName_CN /> 服务的 Docker 镜像？
...
3. 执行命令`docker-compose up -d`启动 <ProductName_CN /> 服务。

4. 通过 NebulaGraph Console 连接 <ProductName_CN /> 后，分别执行命令`SHOW HOSTS GRAPH`、`SHOW HOSTS STORAGE`、`SHOW HOSTS META`查看各服务版本。
...
`docker-compose pull`会同时更新 <ProductName_CN /> 服务和 NebulaGraph Console。
...
用户也可以查看视频快速部署 <ProductName_CN /> 2.0。

[使用 docker compose 部署 <ProductName_CN /> 2.0 和 Web Studio](https://www.bilibili.com/video/BV1T54y1b7pa)（18 分 10 秒）

<iframe src="//player.bilibili.com/player.html?aid=844979052&bvid=BV1T54y1b7pa&cid=319036307&autoplay=0&page=1" scrolling="no" border="0" frameborder="no" framespacing="0" allowfullscreen="true" width="720px" height="480px"> </iframe>
-->
