...
-  <ProductName_CN /> 服务已经部署并启动。详细信息参考 [<ProductName_CN /> 安装部署](../4.deployment-and-installation/1.resource-preparations.md "点击前往 <ProductName_CN /> 安装部署")。
...
3. 进入解压缩的`nebula-dashboard`文件夹，并修改配置文件`config.yaml`。
  ...
  |服务名称|默认端口号|说明|
  |:---|:---|:---|
  |nebula-http-gateway | 8090| 为集群服务提供 HTTP 接口，执行 nGQL 语句与 <ProductName_CN /> 进行交互。|
  ...
  stats-exporter:
    ...
    https: false  # 是否为 HTTPS 端口。
  node-exporter:
    - ip: nebulaHostIP_1 # 部署 <ProductName_CN /> 的机器 IP。
      port: 9100
      https: false # 是否为 HTTPS 端口。
  # - ip: nebulaHostIP_2
  #   port: 9100
