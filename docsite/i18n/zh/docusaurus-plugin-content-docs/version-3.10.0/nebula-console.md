...
NebulaGraph Console 是 <ProductName /> 的原生命令行客户端，用于连接 <ProductName /> 集群并执行查询，同时支持管理参数、导出命令的执行结果、导入测试数据集等功能。
...
<!-- - 直接从 <ProductName /> 安装路径的 `bin` 目录中获取二进制文件 `nebula-console`。-->
...
运行二进制文件 `nebula-console` 连接 <ProductName /> 的命令语法如下：
...
| `-enable_ssl` | 连接 <ProductName /> 时使用 SSL 加密双向认证。 |
...
用户可以使用`:EXIT`或者`:QUIT`从 <ProductName /> 断开连接。为方便使用，NebulaGraph Console 支持使用不带冒号（:）的小写命令，例如`quit`。
