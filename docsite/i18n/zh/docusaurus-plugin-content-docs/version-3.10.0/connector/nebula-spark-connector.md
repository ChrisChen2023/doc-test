...
NebulaGraph Spark Connector 是一个 Spark 连接器，提供通过 Spark 标准形式读写 <ProductName_CN /> 数据的能力。NebulaGraph Spark Connector 由 Reader 和 Writer 两部分组成。

- Reader
  
  提供一个 Spark SQL 接口，用户可以使用该接口编程读取 <ProductName_CN /> 图数据，单次读取一个点或 Edge type 的数据，并将读取的结果组装成 Spark 的 DataFrame。

- Writer

  提供一个 Spark SQL 接口，用户可以使用该接口编程将 DataFrame 格式的数据逐条或批量写入 <ProductName_CN /> 。
...
NebulaGraph Spark Connector、 <ProductName_CN /> 内核版本和 Spark 版本对应关系如下。

| Spark Connector 版本 |  <ProductName_CN /> 版本 | Spark 版本 |
|:----------|:-----------|:-------|
...
- Reader 支持将 <ProductName_CN /> 数据读取成 Graphx 的 VertexRDD 和 EdgeRDD，支持非 Long 型点 ID。

- 统一了 SparkSQL 的扩展数据源，统一采用 DataSourceV2 进行 <ProductName_CN /> 数据扩展。

- 支持`insert`、`update`和`delete`三种写入模式。`insert`模式会插入（覆盖）数据，`update`模式仅会更新已存在的数据，`delete`模式只删除数据。

- 支持与 <ProductName_CN /> 之间的 SSL 加密连接。
...
使用 NebulaGraph Spark Connector 读写 <ProductName_CN /> 时，只需要编写以下代码即可实现。

```scala
# 从 <ProductName_CN /> 读取点边数据。
spark.read.nebula().loadVerticesToDF()
spark.read.nebula().loadEdgesToDF()
 
# 将 dataframe 数据作为点和边写入 <ProductName_CN /> 中。
...
- `NebulaConnectionConfig`是连接 <ProductName_CN /> 的配置，说明如下。

  |参数|是否必须|说明|
  |:---|:---|:---|
  |`withMetaAddress`  |是| 所有 Meta 服务的地址，多个地址用英文逗号（,）隔开，格式为`ip1:port1,ip2:port2,...`。读取数据不需要配置`withGraphAddress`。  |
  |`withConnectionRetry`  |否| NebulaGraph Java Client 连接 <ProductName_CN /> 的重试次数。默认值为`1`。  |
...
- `ReadNebulaConfig`是读取 <ProductName_CN /> 数据的配置，说明如下。

  |参数|是否必须|说明|
  |:---|:---|:---|
  |`withUser`  |否|  <ProductName_CN />用户名。Storage 服务要求身份认证时，需要填写该参数。仅 <ProductName_CN /> 企业版支持该参数。  |
  |`withPasswd`  |否|  <ProductName_CN />用户名对应的密码。Storage 服务要求身份认证时，需要填写该参数。仅 <ProductName_CN /> 企业版支持该参数。  |
  |`withSpace`  |是|   <ProductName_CN /> 图空间名称。  |
  |`withLabel`  |是|   <ProductName_CN /> 图空间内的 Tag 或 Edge type 名称。  |
  ...
  |`withPartitionNum`  |否|  配置读取 <ProductName_CN /> 数据时 Spark 的分区数。默认值为 100。该值的配置最好不超过图空间的的分片数量（partition_num）。|

### 向 <ProductName_CN /> 写入数据

:::note

- DataFrame 中的列会自动作为属性写入 <ProductName_CN /> 。
- 请确保 DataFrame 中的列名和<ProductName_CN />中的属性名一致。若不一致，可通过`DataFrame.withColumnRenamed`方法修改列名。
...
- `WriteNebulaVertexConfig`是写入点的配置，说明如下。
  ...
  |`withVidPolicy`  |否|  写入点 ID 时，采用的映射函数，<ProductName_CN /> 仅支持 HASH。默认不做映射。  |
  |`withVidAsProp`  |否|  DataFrame 中作为点 ID 的列是否也作为属性写入。默认值为`false`。如果配置为`true`，请确保 Tag 中有和`VidField`相同的属性名。  |
  |`withUser`  |否|   <ProductName_CN /> 用户名。若未开启[身份验证](../7.data-security/1.authentication/1.authentication.md)，无需配置用户名和密码。   |
  |`withPasswd`  |否|   <ProductName_CN /> 用户名对应的密码。  |
...
- `WriteNebulaEdgeConfig`是写入边的配置，说明如下。
  ...
  |`withSrcPolicy`  |否| 写入起始点时，采用的映射函数，<ProductName_CN /> 仅支持 HASH。默认不做映射。   |
  |`withDstIdField`  |是| DataFrame 中作为目的点的列。   |
  |`withDstPolicy`  |否| 写入目的点时，采用的映射函数，<ProductName_CN /> 仅支持 HASH。默认不做映射。   |
