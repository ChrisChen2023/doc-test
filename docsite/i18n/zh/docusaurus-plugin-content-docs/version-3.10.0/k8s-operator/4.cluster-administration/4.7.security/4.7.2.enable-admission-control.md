...
已使用 K8s 创建一个集群。具体步骤，参见[创建 <ProductName /> 集群](../4.1.installation/4.1.1.cluster-install.md)。
...
- 添加了`ha-mode`注解的集群需满足高可用模式下的最小副本数：
  ...
  :::note
    ...
    高可用模式是指 <ProductName /> 集群服务的高可用。Storage 服务和 Meta 服务是有状态的服务，其副本数据通过 [Raft](../../../1.introduction/3.nebula-graph-architecture/4.storage-service.md#raft) 协议保持一致性且副本数量不能为偶数。因此，高可用模式下，至少需要 3 个 Storage 服务和 3 个 Meta 服务。Graph 服务为无状态的服务，因此其副本数量可以为偶数，但至少需要 2 个副本。
