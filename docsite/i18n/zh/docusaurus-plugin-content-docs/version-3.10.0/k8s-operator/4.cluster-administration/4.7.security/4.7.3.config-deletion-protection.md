...
NebulaGraph Operator 支持删除保护，以防止<ProductName />集群被意外删除。本文介绍如何为<ProductName />集群配置删除保护。

## 前提条件

- <ProductName />集群已创建。更多信息，请参见[创建<ProductName />集群](../4.1.installation/4.1.1.cluster-install.md)。
- <ProductName />集群已启用准入控制。更多信息，请参见[开启准入控制](4.7.2.enable-admission-control.md)。
