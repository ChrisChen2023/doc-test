# 更新 <ProductName_CN /> 集群的配置

 <ProductName_CN /> 集群中 Meta、Storage、Graph 服务都有各自的默认配置。NebulaGraph Operator 支持自定义集群服务的配置。本文介绍如何修改 <ProductName_CN /> 集群的默认配置。

:::note

暂不支持通过 Helm 自定义 <ProductName_CN /> 集群的配置参数。

:::
## 前提条件

已使用 NebulaGraph Operator 创建一个集群。具体步骤，参见[创建 <ProductName_CN /> 集群](4.1.installation/4.1.1.cluster-install.md)。
