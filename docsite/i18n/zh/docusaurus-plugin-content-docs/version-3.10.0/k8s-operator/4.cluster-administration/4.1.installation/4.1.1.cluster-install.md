# 使用 NebulaGraph Operator 安装 <ProductName_CN /> 集群

采用 NebulaGraph Operator 安装 <ProductName_CN /> 集群，能够实现集群管理的自动化，同时具备自动错误恢复的功能。本文介绍`kubectl apply`和`helm`两种方式来使用 NebulaGraph Operator 安装 <ProductName_CN /> 集群。

:::compatibility 历史版本兼容性

1.x 版本的 NebulaGraph Operator 不兼容 3.x 以下版本的 <ProductName_CN /> 。
...
1. 创建命名空间，用于存放 <ProductName_CN /> 集群相关资源。例如，创建`nebula`命名空间。
...
2. 创建集群的 YAML 配置文件`nebulacluster.yaml`。例如，创建名为`nebula`的集群。
  ...
  ??? info "展开查看集群所有可配置的参数及描述"
      ...
      | 参数                                                        | 默认值                   | 描述                                                                                                                                                                                                           |
      | :---------------------------------------------------------- | :----------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
      | `metadata.name`                                             | -                        | 创建的<ProductName_CN />集群名称。                                                                                                                                                                                |
      ...
      | `spec.imagePullPolicy`                                      |       ` Always`                  | <ProductName_CN />镜像的拉取策略。关于拉取策略详情，请参考 [Image pull policy](https://kubernetes.io/docs/concepts/containers/images/#image-pull-policy)。                                                        |
      ...
      | `spec.metad.licenseManagerURL`                              | -                        | 配置指向 [LM](../../2.get-started/2.2.deploy-lm.md) 的 URL，由 LM 的访问地址和端口（默认端口`9119`）组成。例如，`192.168.8.xxx:9119`。仅适用于创建企业版<ProductName_CN />集群。 |
...
3. 为安装集群所需的配置参数设置环境变量。
  ...
  ```bash
  export NEBULA_CLUSTER_NAME=nebula         # <ProductName_CN /> 集群的名字。
  export NEBULA_CLUSTER_NAMESPACE=nebula    # <ProductName_CN /> 集群所处的命名空间的名字。
  export STORAGE_CLASS_NAME=local-sc         # <ProductName_CN /> 集群的 StorageClass。
  ```

4. 为 <ProductName_CN /> 集群创建命名空间（如已创建，略过此步）。
...
5. 查看创建集群时，`nebula-operator`的`nebula-cluster` chart 的可自定义的配置项。
...
](https://github.com/vesoft-inc/nebula-operator/blob/{{operator.branch}}/charts/nebula-cluster/values.yaml) 查看<ProductName_CN />集群的所有可配置参数。
  ...
  - 执行以下命令查看所有可以配置的参数。
    ...
    ??? info "展开查看可配置的参数描述"
        ...
        | `nebula.reference`                                          | `{"name": "statefulsets.apps", "version": "v1"}`                                                      | 被<ProductName_CN />引用的工作负载。                                                                                                                         |
        ...
        | `nebula.metad.licenseManagerURL`                            | `""`                                                                                                  | LM 访问地址和端口号，用于获取 License 信息。仅适用于创建企业版<ProductName_CN />集群。                                                                                                  |
        ...
        | `nebula.console.username`                                   | `root`                                                                                                | <ProductName_CN />命令行客户端的用户名。详情参见[连接集群](../../2.get-started/2.4.connect-to-cluster.md)。                                      |
        | `nebula.console.password`                                   | `nebula`                                                                                              | <ProductName_CN />命令行客户端的密码。                                                                                                           |
        | `nebula.console.image`                                      | `vesoft/nebula-console`                                                                               | <ProductName_CN />命令行客户端的容器镜像。                                                                                                       |
        | `nebula.console.version`                                    | `latest`                                                                                              | <ProductName_CN />命令行客户端的版本。                                                                                                           |
...
6. 创建 <ProductName_CN /> 集群。

  通过`--set`参数自定义 <ProductName_CN /> 集群配置项的默认值，例如，`--set nebula.storaged.replicas=3`可设置 <ProductName_CN /> 集群中 Storage 服务的副本数为 3。
  ...
  helm install "${NEBULA_CLUSTER_NAME}" nebula-operator/nebula-cluster \
      ...
      # 指定 <ProductName_CN /> 集群所处的命名空间。
      --namespace="${NEBULA_CLUSTER_NAMESPACE}" \
      # 自定义集群名称。
      --set nameOverride="${NEBULA_CLUSTER_NAME}" \
      --set nebula.storageClassName="${STORAGE_CLASS_NAME}" \
      # 指定 <ProductName_CN /> 集群的版本。
      --set nebula.version=v<Var name="nebula_release" /> 
  ```

7. 查看 <ProductName_CN /> 集群 Pod 的启动状态。
