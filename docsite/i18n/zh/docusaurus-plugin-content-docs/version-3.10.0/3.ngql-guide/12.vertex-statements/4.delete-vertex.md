...
<ProductName_CN /> 2.x 默认删除点及关联该点的出边和入边，<ProductName_CN />  <Var name="nebula_release" /> 默认只删除点，不删除该点关联的出边和入边，此时将默认存在悬挂边。
...
<ProductName_CN /> 找到目标点并删除，该目标点的所有邻边（出边和入边）将成为悬挂边。
