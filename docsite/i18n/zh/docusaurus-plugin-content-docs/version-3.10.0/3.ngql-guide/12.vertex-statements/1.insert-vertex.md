...
`INSERT VERTEX`语句可以在 <ProductName /> 实例的指定图空间中插入一个或多个点。
...
- `tag_name`：点关联的 Tag（点类型）。Tag 的创建，详情请参见 [CREATE TAG](../10.tag-statements/1.create-tag.md)。
  ...
  :::caution
    ...
    <ProductName /> <Var name="nebula_release" /> 中默认不支持插入无 Tag 的点。如需使用无 Tag 的点，在集群内所有 Graph 服务的配置文件（`nebula-graphd.conf`）中新增`--graph_use_vertex_key=true`；在所有 Storage 服务的配置文件（`nebula-storaged.conf`）中新增`--use_vertex_key=true`。插入无 Tag 点的命令示例如`INSERT VERTEX VALUES "1":();`。

:::
- `property_name`：需要设置的属性名称。

- `vid`：点 ID。在 <ProductName /> {{ nebula.release }} 中支持字符串和整数，需要在创建图空间时设置，详情请参见 [CREATE SPACE](../9.space-statements/1.create-space.md)。
