...
从<ProductName_CN /> 2.6.0 起支持了新的 [Schema 相关函数](../6.functions-and-expressions/4.schema.md)。以上示例在<ProductName_CN /> {{ nebula.release}} 中的近似写法如下：
    
```ngql
GO FROM "player100" OVER follow YIELD properties($^).name AS startName, properties($$).age AS endAge;
```

 <ProductName_CN /> {{ nebula.release}} 兼容新旧语法。
