...
在成功启动 Studio 后，用户需要配置连接 <ProductName /> 。本文主要描述 Studio 如何连接 <ProductName /> 。

## 前提条件

在连接 <ProductName /> 数据库前，用户需要确认以下信息：

- Studio 已经启动。详细信息参考[部署 Studio](st-ug-deploy.md)。

-  <ProductName /> 的 Graph 服务本机 IP 地址以及服务所用端口。默认端口为 `9669`。

-  <ProductName /> 登录账号信息，包括用户名和密码。
...
2. 在 Studio 的 **配置数据库** 页面上，输入以下信息：
  ...
  - **Graphd IP 地址**：填写 <ProductName /> 的 Graph 服务本机 IP 地址。例如`192.168.10.100`。

    :::note

    - 即使 <ProductName /> 与 Studio 部署在同一台机器上，用户也必须填写这台机器的本机 IP 地址，而不是 `127.0.0.1` 或者 `localhost`。
    - 在新的标签页连接另一个 <ProductName /> 时，会覆盖旧标签页的会话。如果需要同时登录多个 <ProductName />，可以用不同的浏览器或者无痕模式。

:::
  - **Port**：Graphd 服务的端口。默认为`9669`。

  - **用户名** 和 **密码**：根据 <ProductName /> 的身份验证设置填写登录账号和密码。
...
如果需要重新连接 <ProductName />，可以登出后重新配置数据库。
