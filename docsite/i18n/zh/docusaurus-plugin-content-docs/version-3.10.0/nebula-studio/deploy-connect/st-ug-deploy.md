...
1. 根据需要下载 tar 包，建议选择最新版本。
   ...
   | 安装包 | Studio 版本 |适用 <ProductName /> 版本 |
   | --- | --- |---|
...
在命令行工具中按以下步骤依次运行命令，部署并启动 Docker 版 Studio，这里我们用 <ProductName /> 版本为 <Var name="nebula_release" /> 的进行演示：

1. 下载 Studio 的部署配置文件。

   | 安装包 | 适用 <ProductName /> 版本 |
   | ----- | ----- |
...
进入 Studio 登录界面后，用户需要连接 <ProductName /> 。详细信息，参考[连接数据库](st-ug-connect.md)。
