# 查询 <ProductName_CN /> 监控指标

 <ProductName_CN /> 支持多种方式查询服务的监控指标，本文将介绍最基础的方式，即通过 HTTP 端口查询。

## 监控指标结构说明

 <ProductName_CN /> 的每个监控指标都由三个部分组成，中间用英文句号（.）隔开，例如`num_queries.sum.600`。不同的 <ProductName_CN /> 服务支持查询的监控指标也不同。指标结构的说明如下。
...
如果 <ProductName_CN /> 服务部署在容器中，需要执行`docker-compose ps`命令查看映射到容器外部的端口，然后通过该端口查询。
...
如需开启图空间监控指标，先在 Graph 服务的配置文件中将`enable_space_level_metrics`参数的值修改为`true`，再启动 <ProductName_CN /> 。修改配置的详细方式参见[配置管理](../5.configurations-and-logs/1.configurations/1.configurations.md)。
