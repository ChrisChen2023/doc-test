本文介绍如何使用原生命令行客户端 NebulaGraph Console 连接 <ProductName_CN /> 。
...
- NebulaGraph Console 的版本兼容 <ProductName_CN /> 的版本。

  :::note
  
    版本相同的 NebulaGraph Console 和 <ProductName_CN /> 兼容程度最高，版本不同的 NebulaGraph Console 连接 <ProductName_CN /> 时，可能会有兼容问题，或者无法连接并报错`incompatible version between client and server`。
...
6. 执行如下命令连接 <ProductName_CN /> 。
  ...
  | - | - |
  | `-h/-help` | 显示帮助菜单。 |
  | `-addr/-address` | 设置要连接的 Graph 服务的 IP 或主机名。默认地址为 127.0.0.1。<!-- 如果 <ProductName_CN /> 部署在 [Nebula Cloud](https://docs.nebula-graph.com.cn/2.6.2/nebula-cloud/1.what-is-cloud/) 上，需要创建 [Private Link](https://docs.nebula-graph.com.cn/2.6.2/nebula-cloud/5.solution/5.2.connection-configuration-and-use)，并设置该参数的值为专用终结点的 IP 地址。 -->|
  | `-P/-port` | 设置要连接的 Graph 服务的端口。默认端口为 9669。|
  | `-u/-user` | 设置 <ProductName_CN /> 账号的用户名。未启用身份认证时，可以使用任意已存在的用户名（默认为`root`）。 |
  ...
  | `-enable_ssl` | 连接 <ProductName_CN /> 时使用 SSL 加密。 |
