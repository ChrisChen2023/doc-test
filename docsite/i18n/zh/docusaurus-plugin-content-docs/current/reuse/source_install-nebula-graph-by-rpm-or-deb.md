RPM 和 DEB 是 Linux 系统下常见的两种安装包格式，本文介绍如何使用 RPM 或 DEB 文件在一台机器上快速安装 <ProductName_CN /> 。

:::note

部署 <ProductName_CN /> 集群的方式参见[使用 RPM/DEB 包部署集群](https://docs.nebula-graph.com.cn/<Var name="nebula_release" />/2.quick-start/3.quick-start-on-premise/3.1add-storage-hosts/)。<!--这里用外链。-->
...
- 当前仅支持在 Linux 系统下安装 <ProductName_CN />，且仅支持 CentOS 7.x、CentOS 8.x、Ubuntu 16.04、Ubuntu 18.04、Ubuntu 20.04 操作系统。
  
- 如果用户使用的是国产化的 Linux 操作系统，请[安装企业版 <ProductName_CN /> ](https://yueshu.com.cn/contact)。  
...
- 安装 RPM 包
  ...
  `--prefix`为可选项，用于指定安装路径。如不设置，系统会将 <ProductName_CN /> 安装到默认路径`/usr/local/nebula/`。
...
- 安装 DEB 包
  ...
  :::note
    使用 DEB 包安装 <ProductName_CN /> 时不支持自定义安装路径。默认安装路径为`/usr/local/nebula/`。
...
- [启动 <ProductName_CN /> ](https://docs.nebula-graph.com.cn/<Var name="nebula_release" />/2.quick-start/3.quick-start-on-premise/5.start-stop-service/)<!--这里用外链。-->
