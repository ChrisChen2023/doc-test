...
本文以一个示例说明如何使用 Exchange 将自定义类型的数据源导入到 <ProductName />。
...
- `nebulaConfig`：<ProductName /> 服务的配置项。
...
运行如下命令，使用自定义数据源模式将 CSV 文件数据导入到 <ProductName /> 中。关于参数的说明，请参见[导入命令参数](../parameter-reference/ex-ug-para-import-command.md)。
