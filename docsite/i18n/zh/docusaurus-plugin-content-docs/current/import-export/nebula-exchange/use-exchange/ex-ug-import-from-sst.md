...
本文以一个示例说明如何将数据源的数据生成 SST（Sorted String Table）文件并保存在 HDFS 上，然后导入 <ProductName_CN />，示例数据源是 CSV 文件。
...
- 直接将数据源的数据通过** nGQL **语句的形式导入 <ProductName_CN /> 。

- 将数据源的数据生成 SST 文件，然后借助 Console 将 SST 文件导入 <ProductName_CN /> 。
...
## 实现方法
 ...
 <ProductName_CN /> 底层使用 RocksDB 作为键值型存储引擎。RocksDB 是基于硬盘的存储引擎，提供了一系列 API 用于创建及导入 SST 格式的文件，有助于快速导入海量数据。

SST 文件是一个内部包含了任意长度的有序键值对集合的文件，用于高效地存储大量键值型数据。生成 SST 文件的整个过程主要由 Exchange 的 Reader、sstProcessor 和 sstWriter 完成。整个数据处理过程如下：

1. Reader 从数据源中读取数据。

2. sstProcessor 根据 <ProductName_CN /> 的 Schema 信息生成 SST 文件，然后上传至 HDFS。SST 文件的格式请参见[数据存储格式](../../../1.introduction/3.nebula-graph-architecture/4.storage-service.md)。

3. sstWriter 打开一个文件并插入数据。生成 SST 文件时，Key 必须按照顺序写入。

4. 生成 SST 文件之后，RocksDB 通过`IngestExternalFile()`方法将 SST 文件导入到 <ProductName_CN /> 中。例如：
...
- 已经[安装部署 <ProductName_CN /> <Var name="nebula_release" />](../../../4.deployment-and-installation/2.compile-and-install-nebula-graph/2.install-nebula-graph-by-rpm-or-deb.md) 并获取如下信息：
...
- 确认 Hadoop 服务在所有部署 Storage 服务的机器上运行正常。
  ...
  - 如需在 INGEST（数据导入）结束后自动移除 SST 文件，在 Storage 服务配置文件中增加`--move_files=true`，该配置会让 <ProductName_CN /> 在 INGEST 后将 SST 文件移动（`mv`）到`data`目录下。`--move_files`的默认值为`false`，此时 <ProductName_CN /> 会复制（`cp`）SST 文件而不是移动。
...
```conf
{
  ...
  nebula: {
    ...
    # 使用Google Guava RateLimiter 来限制发送到 <ProductName_CN /> 的请求。
    rate: {
      # RateLimiter 的稳定吞吐量。
      limit: 1024
  ...
  tags: [
    # 设置 Tag player 相关信息。
    {
      ...
      # 生成 SST 文件时是否要基于 <ProductName_CN /> 中图空间的 partition 进行数据重分区。
...
:::
使用客户端工具连接 <ProductName_CN />，按如下操作导入 SST 文件：
...
- 如果需要重新下载，请在 <ProductName_CN /> 安装路径内的`data/storage/nebula`目录内，将对应 Space ID 目录内的`download`文件夹删除，然后重新下载 SST 文件。如果图空间是多副本，保存副本的所有机器都需要删除`download`文件夹。
...
### 步骤 7：（如有）在 <ProductName_CN /> 中重建索引
