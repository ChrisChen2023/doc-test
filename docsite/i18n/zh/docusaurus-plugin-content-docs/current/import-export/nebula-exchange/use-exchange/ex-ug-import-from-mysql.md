...
本文以一个示例说明如何使用 Exchange 将存储在 MySQL 上的数据导入 <ProductName_CN />，也适用于从 PostgreSQL 导出数据到 <ProductName_CN /> 。
...
分析数据，按以下步骤在 <ProductName_CN /> 中创建 Schema：
...
2. 在 <ProductName_CN /> 中创建一个图空间** basketballplayer**，并创建一个 Schema，如下所示。
...
# <ProductName_CN /> 相关配置
  nebula: {
    address:{
      # 以下为 <ProductName_CN /> 的 Graph 服务和 Meta 服务所在机器的 IP 地址及端口。
    ...
    # 填写的账号必须拥有 <ProductName_CN /> 相应图空间的写数据权限。
    user: root
    pswd: nebula
    # 是否使用 RSA 加密的密码。
    # enableRSA: true
    # 使用 RSA 加密密码时的密钥。
    # privateKey: ""
    # 填写 <ProductName_CN /> 中需要写入数据的图空间名称。
  ...
  tags: [
    # 设置 Tag player 相关信息。
    {
      ...
      # 在 fields 里指定 player 表中的列名称，其对应的 value 会作为 <ProductName_CN /> 中指定属性。
      # fields 和 nebula.fields 里的配置必须一一对应。
      # 如果需要指定多个列名称，用英文逗号（,）隔开。
      fields: [age,name]
      nebula.fields: [age,name]

      # 指定表中某一列数据为 <ProductName_CN /> 中点 VID 的来源。
      vertex: {
        field:playerid
      # udf:{
      #            separator:"_"
  ...
  edges: [
    ...
    {
      # <ProductName_CN /> 中对应的 Edge type 名称。
      name: follow
      ...
      # 在 fields 里指定 follow 表中的列名称，其对应的 value 会作为 <ProductName_CN /> 中指定属性。
      # fields 和 nebula.fields 里的配置必须一一对应。
      # 如果需要指定多个列名称，用英文逗号（,）隔开。
      fields: [degree]
...
### 步骤 3：向 <ProductName_CN /> 导入数据

运行如下命令将 MySQL 数据导入到 <ProductName_CN /> 中。关于参数的说明，请参见[导入命令参数](../parameter-reference/ex-ug-para-import-command.md)。
...
### 步骤 5：（如有）在 <ProductName_CN /> 中重建索引
