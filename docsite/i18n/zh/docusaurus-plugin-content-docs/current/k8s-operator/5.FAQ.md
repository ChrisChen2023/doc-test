...
## NebulaGraph Operator 支持 v1.x 版本的 <ProductName_CN /> 吗？

不支持，因为 v1.x 版本的 <ProductName_CN /> 不支持 DNS，而 NebulaGraph Operator 需要使用 DNS。
...
## Operator 文档中的 replica 和 <ProductName_CN /> 内核文档中的 replica 是一样的吗？

二者是不同的概念。Operator 文档中的 replica 是 K8s 中的 Pod 副本，而内核文档中的 replica 是 <ProductName_CN /> 中的分片副本。


## 如何查看 <ProductName_CN /> 集群中各服务的日志？
