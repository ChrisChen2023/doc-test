...
5. 拉取最新的 CRD 配置文件。
  ...
  :::note
    ...
    升级 Operator 后，需要同时升级相应的 CRD 配置，否则 <ProductName_CN /> 集群创建会失败。有关 CRD 的配置，参见 [apps.nebula-graph.io_nebulaclusters.yaml](https://github.com/vesoft-inc/nebula-operator/blob/{{operator.tag}}/config/crd/bases/apps.nebula-graph.io_nebulaclusters.yaml)。
