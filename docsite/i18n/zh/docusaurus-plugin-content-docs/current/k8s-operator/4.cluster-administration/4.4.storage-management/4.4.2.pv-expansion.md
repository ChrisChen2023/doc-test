...
在 K8s 环境中，<ProductName /> 的数据存储在持久化存储卷（PV）上。动态在线扩容存储卷指的是在不停机的情况下增加存储卷的容量，以满足 <ProductName /> 数据增长的需求。本文介绍如何在 K8s 环境中为 <ProductName /> 的服务动态在线扩容存储卷。
...
在 Operator 中，不能直接编辑 PVC，因为 Operator 会根据 <ProductName /> 集群服务的配置`spec.<metad|storaged>.dataVolumeClaim`自动创建 PVC。因此，需要通过修改集群的配置来实现 PVC 的配置更新，然后自动触发 PV 的动态在线扩容。
...
- 在 K8s 中创建一个 <ProductName /> 集群。具体步骤，参见[创建 <ProductName /> 集群](../4.1.installation/4.1.1.cluster-install.md)。
-  <ProductName /> 集群 Pod 处于运行状态。

## 在线扩容存储卷示例

以下示例假设 StorageClass 的名称为`ebs-sc`，<ProductName /> 集群的名称为`nebula`，演示如何在线扩容 Storage 服务的存储卷。
