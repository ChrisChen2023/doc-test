...
本文介绍如何删除使用 NebulaGraph Operator 创建的 <ProductName_CN /> 集群。

## 使用限制

- 只支持删除使用 NebulaGraph Operator 创建的<ProductName_CN />集群。
- 无法删除启用了删除保护的集群。更多信息，请参见[配置删除保护](../4.7.security/4.7.3.config-deletion-protection.md)。

## 使用`kubectl`删除 <ProductName_CN /> 集群
