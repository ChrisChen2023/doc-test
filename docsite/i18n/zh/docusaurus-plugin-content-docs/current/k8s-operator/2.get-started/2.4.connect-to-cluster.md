# 连接 <ProductName_CN /> 集群

使用 NebulaGraph Operator 创建 <ProductName_CN /> 集群后，用户可在 <ProductName_CN /> 集群内部访问 <ProductName_CN />，也可在集群外访问 <ProductName_CN /> 。如果用户在集群内部，可以通过访问集群内的虚拟 IP 地址（ClusterIP）访问数据库。而如果用户在集群外部，则需要使用集群节点的公共 IP 地址或者通过部署 Nginx Ingress 控制器来访问 <ProductName_CN /> 。IP 地址类型可以在创建集群的配置文件中，通过`spec.graphd.service`指定。关于 Service 的更多信息，参考 [Service](https://kubernetes.io/docs/concepts/services-networking/service/)。

## 前提条件

已使用 NebulaGraph Operator 创建 <ProductName_CN /> 集群。具体步骤参考[快速创建集群](2.3.create-cluster.md)。

## 在 <ProductName_CN /> 集群内连接 <ProductName_CN /> 

用户可以创建`ClusterIP`类型的 Service，为集群内的其他 Pod 提供访问 <ProductName_CN /> 的入口。通过该 Service 的 IP 和数据库 Graph 服务的端口号（`9669`），可连接 <ProductName_CN /> 。更多信息，请参考 [ClusterIP](https://kubernetes.io/docs/concepts/services-networking/service/)。
...
1. 创建名为`graphd-clusterip-service.yaml`的文件。示例内容如下：
  ...
  -  <ProductName_CN /> 默认使用`9669`端口为客户端提供服务。`19669`为 Graph 服务的 HTTP 端口号。
...
4. 使用上述`<cluster-name>-graphd-svc` Service 的 IP 连接 <ProductName_CN /> ：
...
- `<service_port>`为 Graph 服务默认的端口`9669`。
  ...
  :::note
    ...
    如果在集群配置文件中设置了`spec.console`字段，您还可以使用以下命令连接到 <ProductName_CN /> 数据库：
...
:::
## 通过`NodePort`在 <ProductName_CN /> 集群外部连接 <ProductName_CN /> 
...
3. 查看 Service 中 <ProductName_CN /> 映射至集群节点的端口。
...
4. 使用节点 IP 和上述映射的节点端口连接 <ProductName_CN /> 。
  ...
  - `--image`：为连接 <ProductName_CN /> 的工具 Console 的镜像。
  - `<nebula-console>`：自定义的 Pod 名称。本示例为`nebula-console`。
  - `-addr`： <ProductName_CN /> 集群中任一节点 IP 地址。本示例为`192.168.8.24`。
  - `-port`： <ProductName_CN /> 映射至节点的端口。本示例为`32236`。
  - `-u`： <ProductName_CN /> 账号的用户名。未启用身份认证时，可以使用任意已存在的用户名（默认为 root）。
  - `-p`：用户名对应的密码。未启用身份认证时，密码可以填写任意字符。
...
:::
## 通过`Ingress`在 <ProductName_CN /> 集群外部连接 <ProductName_CN /> 
...
用户可以通过 HostNetwork 和 DaemonSet 组合的模式使用 Nginx Ingress 从集群外部连接 <ProductName_CN /> 集群。
...
3. 执行以下命令使 Nginx Ingress 在集群中生效。
  ...
  成功部署 Nginx Ingress 后，由于 Nginx Ingress 中配置的网络类型为`hostNetwork`，因此用户可通过部署了 Nginx Ingress 的节点的 IP（`192.168.8.160`）和外部端口（`9769`）访问 <ProductName_CN /> 服务。

4. 执行以下命令部署连接 <ProductName_CN /> 服务的 Console 并通过宿主机 IP（本示例为`192.168.8.160`）和上述配置的外部端口访问 <ProductName_CN /> 服务。
