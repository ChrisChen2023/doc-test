...
本文将介绍创建 <ProductName /> 集群的两种方式：

- 使用 Helm 创建 <ProductName /> 集群
- 使用 Kubectl 创建 <ProductName /> 集群
...
:::
下面以创建名为`nebula`的集群为例，说明如何部署 <ProductName /> 集群。
...
2. 创建集群配置文件，例如`nebulacluster.yaml`。
  ...
  关于其它参数的详情，请参考[创建 <ProductName /> 集群](../4.cluster-administration/4.1.installation/4.1.1.cluster-install.md)。
