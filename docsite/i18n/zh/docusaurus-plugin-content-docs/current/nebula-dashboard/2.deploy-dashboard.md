...
-  <ProductName /> 服务已经部署并启动。详细信息参考 [<ProductName /> 安装部署](../4.deployment-and-installation/1.resource-preparations.md "点击前往 <ProductName /> 安装部署")。
...
3. 进入解压缩的`nebula-dashboard`文件夹，并修改配置文件`config.yaml`。
  ...
  |服务名称|默认端口号|说明|
  |:---|:---|:---|
  |nebula-http-gateway | 8090| 为集群服务提供 HTTP 接口，执行 nGQL 语句与 <ProductName /> 进行交互。|
