...
- 已经[部署 <ProductName /> 集群](../2.compile-and-install-nebula-graph/deploy-nebula-graph-cluster.md)。
...
- 请保证 <ProductName /> 各服务（Metad、Storaged、Graphd、listener）有相同的版本。

- 请保证 listener 所在机器的时区与 <ProductName /> 各服务所在机器的时区一致。
...
listener 服务与 storaged 服务使用相同的二进制文件，但是二者配置文件不同，进程使用端口也不同。我们在所有需要部署 listener 的服务器上都安装 <ProductName />，但是不启动服务。详情请参见[使用 RPM 或 DEB 安装包安装 <ProductName /> ](../2.compile-and-install-nebula-graph/2.install-nebula-graph-by-rpm-or-deb.md)。
...
### 第四步：添加 listener 到 <ProductName /> 集群

[用命令行连接到 <ProductName /> ](../../2.quick-start/3.quick-start-on-premise/3.connect-to-nebula-graph.md)，然后执行 [`USE <space>`](../../3.ngql-guide/9.space-statements/2.use-space.md) 进入需要创建全文索引的图空间。然后执行如下命令添加 listener：
