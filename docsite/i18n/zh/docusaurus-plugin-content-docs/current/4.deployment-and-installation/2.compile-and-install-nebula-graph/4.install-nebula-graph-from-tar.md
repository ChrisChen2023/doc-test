# 使用 tar.gz 文件安装 <ProductName /> 

用户可以下载打包好的 tar.gz 文件快速安装 <ProductName /> 。

:::note

-  <ProductName /> 从 2.6.0 版本起提供 tar.gz 文件。
  
- 当前仅支持在 Linux 系统下安装 <ProductName />，且仅支持 CentOS 7.x、CentOS 8.x、Ubuntu 16.04、Ubuntu 18.04、Ubuntu 20.04 操作系统。
  
- 如果用户使用的是国产化的 Linux 操作系统，请[安装 <ProductName /> ](https://www.nebula-graph.com.cn/contact)。 
...
1. 使用如下地址下载 <ProductName /> 的 tar.gz 文件。
  ...
  例如，要下载适用于`CentOS 7.5`的 <ProductName /> {{nebula.branch}} tar.gz 文件，运行以下命令：
...
2. 解压 tar.gz 文件到 <ProductName /> 安装目录。
...
3. 修改配置文件名以应用配置。
  ...
  进入解压出的目录，将子目录`etc`中的文件`nebula-graphd.conf.default`、`nebula-metad.conf.default`和`nebula-storaged.conf.default`重命名，删除`.default`，即可应用 <ProductName /> 的默认配置。
...
[管理 <ProductName /> 服务](../manage-service.md)
