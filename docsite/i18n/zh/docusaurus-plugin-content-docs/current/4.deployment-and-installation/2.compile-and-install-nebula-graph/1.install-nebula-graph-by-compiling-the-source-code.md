...
使用源码安装 <ProductName /> 允许自定义编译和安装设置，并测试最新特性。
...
- 准备正确的编译环境。参见[软硬件要求和安装三方库依赖包](../1.resource-preparations.md)。
  ...
  :::note
    ...
    暂不支持离线编译 <ProductName /> 。

:::
- 待安装 <ProductName /> 的主机可以访问互联网。



## 安装步骤

1. 克隆 <ProductName /> 的源代码到主机。

  - [推荐] 如果需要安装<Var name="nebula_release" />版本的 <ProductName />，执行如下命令：
...
2. 进入`nebula/third-party`目录，安装 <ProductName /> 依赖的第三方库。
...
master 分支的代码更新速度快，如果安装了 master 分支对应的开发版 <ProductName />，可根据以下步骤更新版本。
...
[管理 <ProductName /> 服务](../../2.quick-start/3.quick-start-on-premise/5.start-stop-service.md)
...
`CMAKE_INSTALL_PREFIX`指定 <ProductName /> 服务模块、脚本和配置文件的安装路径，默认路径为`/usr/local/nebula`。
...
默认值为`ON`，表示单元测试服务由 <ProductName /> 服务构建。如果只需要服务模块，可以设置为`OFF`。

### ENABLE_ASAN

默认值为`OFF`，表示关闭内存问题检测工具 ASan（AddressSanitizer）。该工具是为 <ProductName /> 开发者准备的，如果需要开启，可以设置为`ON`。

### CMAKE_BUILD_TYPE

`CMAKE_BUILD_TYPE`控制 <ProductName /> 的 build 方法，取值说明如下：
