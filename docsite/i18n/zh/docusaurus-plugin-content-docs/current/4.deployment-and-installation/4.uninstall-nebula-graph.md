...
本文介绍如何卸载 <ProductName_CN /> 。

:::caution

如果需要重新部署 <ProductName_CN />，请务必完全卸载后再重新部署，否则可能会出现问题，包括 Meta 不一致等。

:::
## 前提条件

停止 <ProductName_CN /> 服务。详情参见[管理 <ProductName_CN /> 服务](../2.quick-start/3.quick-start-on-premise/5.start-stop-service.md)。
...
### 卸载编译安装的 <ProductName_CN /> 

找到 <ProductName_CN /> 的安装目录，删除整个安装目录。

### 卸载 RPM 包安装的 <ProductName_CN /> 

1. 使用如下命令查看 <ProductName_CN /> 版本。
...
### 卸载 Docker Compose 部署的 <ProductName_CN /> 

1. 在目录`nebula-docker-compose`内执行如下命令停止 <ProductName_CN /> 服务。
