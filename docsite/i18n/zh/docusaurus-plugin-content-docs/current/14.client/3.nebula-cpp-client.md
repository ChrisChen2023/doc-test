...
[NebulaGraph CPP](https://github.com/vesoft-inc/nebula-cpp/tree/{{cpp.branch}}) 是一款 C++ 语言的客户端，可以连接、管理 <ProductName_CN /> 图数据库。
...
2. 编译文件，命令如下：
  ...
  - `library_folder_path`： <ProductName_CN /> 动态库文件存储路径，默认为`/usr/local/nebula/lib64`。

  - `include_folder_path`： <ProductName_CN /> 头文件存储路径，默认为`/usr/local/nebula/include`。
...
NebulaGraph CPP 客户端提供 Session Pool 和 Connection Pool 两种方式连接 <ProductName_CN /> 。使用 Connection Pool 需要用户自行管理 Session 实例。
