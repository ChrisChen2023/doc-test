# <ProductName_CN /> 查询语句概述

本文介绍 <ProductName_CN /> 的通用的查询语句分类及各类语句的使用场景。

## 背景信息

<ProductName_CN /> 的数据以点和边的形式存储。每个点可以有 0 或多个标签（Tag）；每条边有且仅有一个边类型（Edge Type）。标签定义点的类型以及描述点的属性；边类型定义边的类型以及描述边的属性。在查询时，可以通过指定点的标签或边的类型来限定查询的范围。更多信息，请参见[数据模型](../1.nGQL-overview/3.graph-patterns.md)。

## 查询语句分类

<ProductName_CN /> 的核心查询语句可分为：
<!-- no toc -->
...
<ProductName_CN /> 的查询语句可以组合使用，以实现更复杂的查询。
