# 管理快照
 ...
 <ProductName_CN /> 提供快照（snapshot）功能，用于保存集群当前时间点的数据状态，当出现数据丢失或误操作时，可以通过快照恢复数据。

## 前提条件

 <ProductName_CN /> 的[身份认证](../7.data-security/1.authentication/1.authentication.md)功能默认是关闭的，此时任何用户都能使用快照功能。
...
```bash
$ cd /usr/local/yueshu-graph-<Var name="nebula_release" />/data
...
除了使用快照，用户还可以使用备份恢复工具 Backup&Restore（BR）备份或恢复 <ProductName_CN /> 数据。详情请参见 [Backup&Restore](2.backup-restore/1.what-is-br.md)。
-->
