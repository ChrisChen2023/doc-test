...
Backup&Restore（简称 BR）是一款命令行界面（CLI）工具，可以帮助备份 <ProductName_CN /> 的图空间数据，或者通过备份文件恢复数据。
...
- 支持备份并恢复整个 <ProductName_CN /> 集群。
- （实验性功能）支持备份指定图空间数据。

## 限制

-  <ProductName_CN /> 版本需要为 v3.x。
- 数据备份仅支持全量备份，不支持增量备份。
...
- 数据恢复期间有一段时间服务会被停止。
- 不支持在容器部署的 <ProductName_CN /> 集群中使用 BR。
...
- [<ProductName_CN /> 容灾备份工具 nebula-br 介绍](https://www.bilibili.com/video/av851858570)（3 分 34 秒）
