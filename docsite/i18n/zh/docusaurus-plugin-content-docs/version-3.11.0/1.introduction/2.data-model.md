...
本文介绍 <ProductName /> 的数据模型。数据模型是一种组织数据并说明它们如何相互关联的模型。

## 数据模型

<ProductName /> 数据模型使用 6 种基本的数据模型：
...
- 点（Vertex）
  ...
  :::compatibility
    ...
    <ProductName /> 2.x 及以下版本中的点必须包含至少一个 Tag。
...
<ProductName /> 使用有向属性图模型，指点和边构成的图，这些边是有方向的，点和边都可以有属性。
