...
使用 BR 工具备份和恢复 <ProductName_CN /> 时，需要安装 NebulaGraph Agent 服务。Agent 是集群中每台机器的一个守护进程，用于启停 <ProductName_CN /> 服务和上传、下载备份文件。BR 工具和 Agent 插件的安装方式如下文。

## 版本兼容性

| <ProductName_CN /> |BR 社区版 |Agent |
|:---|:---|:---|
...
4. 执行以下命令启动 Agent。
  ...
  :::note
    ...
    启动 Agent 前，确保已经启动集群中的 Meta 服务，并且 Agent 有对应 <ProductName_CN /> 服务目录和备份目录的读写权限。
