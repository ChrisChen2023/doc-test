...
Solution 可在 Connectivity 中设置 Private Link 让用户能够通过虚拟网络中的专用节点访问 <ProductName_CN />，并且网络流量不会进入公共互联网。
...
:::
## 如何使用私有链接连接 <ProductName_CN /> 

用户可以使用私有链接通过 NebulaGraph Console 连接 <ProductName_CN /> 。详情信息，参见[连接 <ProductName_CN /> ](../../2.quick-start/3.quick-start-on-premise/3.connect-to-nebula-graph.md)。
