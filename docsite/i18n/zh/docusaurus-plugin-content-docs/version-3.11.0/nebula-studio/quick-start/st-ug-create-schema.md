...
在 <ProductName_CN /> 中，用户必须先有 Schema，才能向其中写入点数据和边数据。本文描述如何使用 <ProductName_CN /> 的**控制台**或 **Schema** 功能创建 Schema。

:::note

- 用户可以使用 nebula-console 创建 Schema。详情参见 [<ProductName_CN /> 使用手册](../../README.md)和 [<ProductName_CN /> 快速开始](../../2.quick-start/1.quick-start-overview.md)。
...
- Studio 已经连接到 <ProductName_CN /> 数据库。

- 账号拥有 GOD、ADMIN 或 DBA 权限。详细信息，参考 [<ProductName_CN /> 内置角色](../../7.data-security/1.authentication/3.role-list.md)。
