...
| <ProductName_CN /> 许可证费用 | **试用期内免费** | 用户承担 |
| <ProductName_CN /> 服务许可证有效期 | 30 天 | 用户创建实例时选择 |
...
在创建服务实例时，系统会默认开启数据盘自动快照备份，用于周期性备份 <ProductName_CN /> 数据。
...
| 套餐版本 | 说明 |
| - | - |
| 基础版 | 将所有 <ProductName_CN /> 服务节点部署在 1 台 ECS 服务器上。 |
...
2. 打开云市场的 [<ProductName_CN /> 产品页](https://market.aliyun.com/products/56024006/cmgj00059955.html?#sku=yuncode5395500004)。

3. 选择**套餐版本**和**购买时长**（即 <ProductName_CN /> 服务的许可证有效期），并单击**立即购买**。
...
4. 在**创建服务实例**页，保持**选择模板**处的选择不变。如需切换部署架构，可改变选中的选项，重新选择模板。
  ...
  :::note
    ...
    改变模板会改变之前选择的套餐版本，软件费用（ <ProductName_CN /> 许可证费用）和创建服务实例需要的资源也会改变。
...
8. 在 ** <ProductName_CN /> 配置**区域，完成数据盘和 ECS 服务器密码设置。
...
2. 在 ** <ProductName_CN /> 集群版**卡片上，单击**免费试用**。

3. 系统会为首次创建 <ProductName_CN /> 服务实例的用户弹出申请对话框。在**申请权限**对话框中填写申请人信息。带有红色星号（*）的为必填项。
...
6. 在 ** <ProductName_CN /> 配置**区域，完成数据盘和 ECS 服务器密码设置。
...
3. 如果仍然创建失败，到 [<ProductName_CN /> 论坛](https://discuss.nebula-graph.com.cn/)寻求帮助。
