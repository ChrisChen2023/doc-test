RPM 和 DEB 是 Linux 系统下常见的两种安装包格式，本文介绍如何使用 RPM 或 DEB 文件在一台机器上快速安装 <ProductName /> 。

:::note

部署 <ProductName /> 集群的方式参见[使用 RPM/DEB 包部署集群](https://docs.nebula-graph.com.cn/<Var name="nebula_release" />/2.quick-start/3.quick-start-on-premise/3.1add-storage-hosts/)。<!--这里用外链。-->
...
- 当前仅支持在 Linux 系统下安装 <ProductName />，且仅支持 CentOS 7.x、CentOS 8.x、Ubuntu 16.04、Ubuntu 18.04、Ubuntu 20.04 操作系统。
  
- 如果用户使用的是国产化的 Linux 操作系统，请[安装企业版 <ProductName /> ](https://yueshu.com.cn/contact)。  
...
- 安装 RPM 包
  ...
  `--prefix`为可选项，用于指定安装路径。如不设置，系统会将 <ProductName /> 安装到默认路径`/usr/local/nebula/`。

  例如，要在默认路径下安装<Var name="nebula_release" />版本的 RPM 包，运行如下命令：
...
- 安装 DEB 包
  ...
  :::note
    使用 DEB 包安装 <ProductName /> 时不支持自定义安装路径。默认安装路径为`/usr/local/nebula/`。

:::
  例如安装<Var name="nebula_release" />版本的 DEB 包：
...
- [启动 <ProductName /> ](https://docs.nebula-graph.com.cn/<Var name="nebula_release" />/2.quick-start/3.quick-start-on-premise/5.start-stop-service/)<!--这里用外链。-->
- [连接 <ProductName /> ](https://docs.nebula-graph.com.cn/<Var name="nebula_release" />/2.quick-start/3.quick-start-on-premise/3.connect-to-nebula-graph/)<!--这里用外链。-->
