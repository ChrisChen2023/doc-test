...
如果存储在 <ProductName_CN /> 中的数据有变化，为了获取最新的统计结果，请重新执行`SUBMIT JOB STATS`。
...
`SUBMIT JOB DOWNLOAD HDFS`和`SUBMIT JOB INGEST`命令用于将 SST 文件导入 <ProductName_CN /> 。详情参见[导入 SST 文件数据](../import-export/nebula-exchange/use-exchange/ex-ug-import-from-sst.md)。
