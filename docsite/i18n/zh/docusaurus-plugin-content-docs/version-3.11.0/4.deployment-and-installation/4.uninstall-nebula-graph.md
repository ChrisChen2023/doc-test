...
本文介绍如何卸载 <ProductName /> 。

:::caution

如果需要重新部署 <ProductName />，请务必完全卸载后再重新部署，否则可能会出现问题，包括 Meta 不一致等。

:::
## 前提条件

停止 <ProductName /> 服务。详情参见[管理 <ProductName /> 服务](../2.quick-start/3.quick-start-on-premise/5.start-stop-service.md)。
...
找到 <ProductName /> 的安装目录，删除整个安装目录。

### 卸载 RPM 包安装的 <ProductName /> 

1. 使用如下命令查看 <ProductName /> 版本。
...
### 卸载 Docker Compose 部署的 <ProductName /> 

1. 在目录`nebula-docker-compose`内执行如下命令停止 <ProductName /> 服务。
