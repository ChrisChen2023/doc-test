...
NebulaGraph Operator 调用 <ProductName_CN /> 集群提供的接口，动态地感知服务是否正常运行。当 <ProductName_CN /> 集群中某一组件停止运行时，NebulaGraph Operator 会自动地进行容错处理。本文通过删除 <ProductName_CN /> 集群中 1 个 Storage 服务 Pod，模拟集群故障为例，说明 Nebular Operator 如何进行故障自愈。
...
1. 创建 <ProductName_CN /> 集群。具体步骤参考[创建 <ProductName_CN /> 集群](../4.1.installation/4.1.1.cluster-install.md)。
...
2. 待所有 Pods 都处于`Running`状态时，模拟故障，删除名为`<cluster_name>-storaged-2` Pod。
  ...
  ```
`<cluster_name>`为 <ProductName_CN /> 集群的名称。
