# 使用 NebulaGraph Operator 升级 <ProductName /> 集群版本

本文介绍如何使用 NebulaGraph Operator 在 Kubernetes 环境中升级 <ProductName /> 集群的版本。
...
- 只支持升级使用 NebulaGraph Operator 创建的 <ProductName /> 集群。

- 只支持升级 <ProductName /> {{operator.upgrade_from}} 至 {{operator.upgrade_to}} 版本。

- 如需升级 <ProductName /> 企业版，[联系我们](https://discuss.nebula-graph.com.cn/uploads/default/optimized/3X/7/6/76add3b56ce137be42ebf6e79027bef8a1a0de6b_2_648x1000.jpeg)。

## 前提条件

已创建 <ProductName /> 集群。具体步骤，参见[创建 <ProductName /> 集群](4.1.1.cluster-install.md)。

## 使用`kubectl`升级 <ProductName /> 集群

本示例中待升级的 <ProductName /> 版本为 {{operator.upgrade_from}}。
...
3. 升级 <ProductName /> 集群。
  ...
  例如升级至 {{nebula.tag}} <ProductName /> 集群的命令如下。
