...
[NebulaGraph Operator](../1.introduction-to-nebula-operator.md) 为用户管理 <ProductName /> 集群，使用户无需在生产环境中手动安装、扩展、升级和卸载 NebulaGraph，减轻用户管理不同应用版本的负担。
...
- [CoreDNS](https://coredns.io/) 是一个灵活的、可扩展的 DNS 服务器，被[安装](https://github.com/coredns/helm)在集群内作为集群内 Pods 的 DNS 服务器。 <ProductName /> 集群中的每个组件通过 DNS 解析类似`x.default.svc.cluster.local`这样的域名相互通信。
