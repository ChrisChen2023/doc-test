# 配置管理
 ...
 <ProductName /> 基于 [gflags](https://gflags.github.io/gflags/) 库打造了系统配置，多数配置项都是其中的 flags。 <ProductName /> 服务启动时，默认会从[配置文件](#_4)中获取配置信息。对于文件中没有的配置项，系统使用默认值。



:::note

* 由于配置项多且可能随着 <ProductName /> 的开发发生变化，文档不会介绍所有配置项。按下文说明可在命令行获取配置项的详细说明。
...
以上示例使用了二进制文件的默认存储路径`/usr/local/nebula/bin/`。如果修改了 <ProductName /> 安装路径，使用实际路径查询配置项信息。

## 查看运行配置

使用`curl`命令获取运行中的配置项的值，即 <ProductName /> 的运行配置。
...
### 源码、RPM/DEB、TAR 包集群的配置文件
 ...
 <ProductName /> 为每个服务都提供了两份初始配置文件`<service_name>.conf.default`和`<service_name>.conf.production`，方便用户在不同场景中使用。使用源码和 RPM/DEB 安装集群的配置文件的默认路径为`/usr/local/nebula/etc/`；使用 TAR 包安装集群的配置文件路径为`<install_path>/<tar_package_directory>/etc`TAR 包的安装路径。

初始配置文件中的配置值仅供参考，使用时可根据实际需求调整。如需使用初始配置文件，从上述两个文件选择其一，删除后缀`.default`或`.production`使其生效。

:::note

为确保服务的可用性，同类服务的配置建议保持一致，本机`local_ip`除外。例如，一个 <ProductName /> 集群中部署了 3 个 Storage 服务器，3 者除`local_ip`外的其它配置建议都相同。

:::
下表列出了各服务对应的初始配置文件。

|  <ProductName /> 服务 | 初始配置文件 | 配置说明 |
| - | - | - |
...
所有服务的初始配置文件中都包含`local_config`参数，预设值为`true`，表示 <ProductName /> 服务会从其配置文件获取配置并启动。

:::caution

不建议修改`local_config`的值为`false`。修改后 <ProductName /> 服务启动后会先尝试从 Meta 服务获取缓存的配置信息，可能导致集群配置不一致，造成未知风险。
...
:::
关于 <ProductName /> 的配置相关的更多信息，参见 [<ProductName /> Config](https://siwei.io/sketches/nebula-config-explained/)。

## 修改配置

用户可以在配置文件中修改 <ProductName /> 的配置，或使用命令动态修改配置。
...
默认情况下，所有 <ProductName /> 服务从配置文件获取配置。用户可以按照以下步骤修改配置并使其生效。

* 针对使用源码、RPM/DEB、TAR 包安装的集群：

  1. 使用文本编辑器修改目标服务的配置文件并保存。

  2. 选择合适的时间重启**所有** <ProductName /> 服务使修改生效。
...
用户可以通过`curl`命令动态修改 <ProductName /> 服务的配置。例如，修改 Storage 服务的`wal_ttl`参数为`600`，命令如下：
