...
用户在配置文件里设置连接 <ProductName /> 的密码时，可以使用明文密码，也可以使用 RSA 加密后的密码。
...
|`nebula.user`|string|-|是|拥有 <ProductName /> 写权限的用户名。|
...
:::note
 ...
 <ProductName /> 默认不支持无 Tag 的点。如果需要导入无 Tag 的点，需要先在集群内开启[支持无 Tag 点](../../../3.ngql-guide/12.vertex-statements/1.insert-vertex.md)，然后在 Exchange 的配置文件内新增`nebula.enableTagless`参数，值为`true`。示例如下：
...
|参数|数据类型|默认值|是否必须|说明|
|:---|:---|:---|:---|:---|
|`tags.name`|string|-|是| <ProductName /> 中定义的 Tag 名称。|
...
|`tags.nebula.fields`|list\[string\]|-|是| <ProductName /> 中定义的属性名称，顺序必须和`tags.fields`一一对应。例如`[_c1, _c2]`对应`[name, age]`，表示第二列为属性 name 的值，第三列为属性 age 的值。|
...
|`tags.batch`|int|`256`|是|单批次写入<ProductName />的最大点数量。|
|`tags.partition`|int|`32`|是|数据写入<ProductName />时需要创建的分区数。如果`tags.partition ≤ 1`，在<ProductName />中创建的分区数和数据源的分区数相同。|
|`tags.filter`|string|-|否|过滤规则。符合过滤规则的数据会被导入<ProductName />。格式请参见[Dataset](https://spark.apache.org/docs/latest/api/scala/org/apache/spark/sql/Dataset.html#filter(conditionExpr:String):org.apache.spark.sql.Dataset[T])。|
...
|`tags.repartitionWithNebula`|bool|`true`|否|生成 SST 文件时是否要基于 <ProductName /> 中图空间的 partition 进行数据重分区。开启该功能可减少 DOWNLOAD 和 INGEST SST 文件需要的时间。|
...
|参数|数据类型|默认值|是否必须|说明|
|:---|:---|:---|:---|:---|
|`edges.name`| string|-|是| <ProductName /> 中定义的 Edge type 名称。|
...
|`edges.nebula.fields`|list\[string\]|-|是| <ProductName /> 中定义的属性名称，顺序必须和`edges.fields`一一对应。例如`[_c2, _c3]`对应`[start_year, end_year]`，表示第三列为开始年份的值，第四列为结束年份的值。|
...
|`edges.batch`|int|`256`|是|单批次写入<ProductName />的最大边数量。|
|`edges.partition`|int|`32`|是|数据写入<ProductName />时需要创建的分区数。如果`edges.partition ≤ 1`，在<ProductName />中创建的分区数和数据源的分区数相同。|
|`edges.filter`|string|-|否|过滤规则。符合过滤规则的数据会被导入<ProductName />。格式请参见[Dataset](https://spark.apache.org/docs/latest/api/scala/org/apache/spark/sql/Dataset.html#filter(conditionExpr:String):org.apache.spark.sql.Dataset[T])。|
...
|`edges.repartitionWithNebula`|bool|`true`|否|生成 SST 文件时是否要基于 <ProductName /> 中图空间的 partition 进行数据重分区。开启该功能可减少 DOWNLOAD 和 INGEST SST 文件需要的时间。|
