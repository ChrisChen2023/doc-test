...
分析 ORC 文件中的数据，按以下步骤在 <ProductName /> 中创建 Schema：
...
```conf
{
  ...
  tags: [
    # 设置 Tag player 相关信息。
    {
      ...
      # 在 fields 里指定 ORC 文件中 key 名称，其对应的 value 会作为 <ProductName /> 中指定属性的数据源。
      # 如果需要指定多个值，用英文逗号（,）隔开。
      fields: [age,name]
...
运行如下命令将 ORC 文件数据导入到 <ProductName /> 中。关于参数的说明，请参见[导入命令参数](../parameter-reference/ex-ug-para-import-command.md)。
