...
JDBC 数据是指用 JDBC 接口访问的各类数据库的数据的统称。本文以 MySQL 数据库为例说明如何使用 Exchange 将 JDBC 数据导入 <ProductName_CN /> 。
...
分析文件中的数据，按以下步骤在 <ProductName_CN /> 中创建 Schema：
...
运行如下命令将 JDBC 数据导入到 <ProductName_CN /> 中。关于参数的说明，请参见[导入命令参数](../parameter-reference/ex-ug-para-import-command.md)。
