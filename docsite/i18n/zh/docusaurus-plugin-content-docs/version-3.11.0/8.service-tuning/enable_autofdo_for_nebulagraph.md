...
AutoFDO 可以对优化过的程序进行性能分析，并使用性能信息来指导编译器再次优化程序。本文将帮助您为 <ProductName_CN /> 启用AutoFDO。
...
### 编译 <ProductName_CN /> 二进制文件

关于如何从源码编译 <ProductName_CN />，请参考[使用源码安装 <ProductName_CN /> ](../4.deployment-and-installation/2.compile-and-install-nebula-graph/1.install-nebula-graph-by-compiling-the-source-code.md)。
...
## 使用合并的性能数据文件重新编译 <ProductName_CN /> 二进制文件

使用编译选项`-fauto-profile`重新编译 <ProductName_CN /> 二进制文件。
