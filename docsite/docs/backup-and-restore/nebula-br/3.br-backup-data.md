...
| `--storage` | string | Yes | None | The target storage URL of BR backup data. The format is: `<Schema>://<PATH>` where Schema can be `local` or `s3`. When selecting s3, you need to fill in `s3.access_key`, `s3.endpoint`, `s3.region`, and `s3.secret_key`. PATH is the path of the storage location. |
