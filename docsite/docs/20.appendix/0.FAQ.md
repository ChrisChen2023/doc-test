...
The error may be caused if the system memory usage is higher than the threshold specified by`system_memory_high_watermark_ratio`, which defaults to `0.8`. When the threshold is exceeded, an alarm is triggered and NebulaGraph stops processing queries.
