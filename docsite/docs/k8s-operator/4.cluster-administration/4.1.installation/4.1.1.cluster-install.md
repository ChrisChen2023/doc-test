...
2. Create a YAML configuration file `nebulacluster.yaml` for the cluster. For example, create a cluster named `nebula`.
  ...
  ??? info "Expand to view all configurable parameters and descriptions"
      ...
      | `spec.metad.licenseManagerURL`                              | -                        | Configures the URL pointing to the License Manager (LM), consisting of the access address and port (default port `9119`). For example, `192.168.8.xxx:9119`. For creating the NebulaGraph Enterprise Edition only.                                                                                        |
