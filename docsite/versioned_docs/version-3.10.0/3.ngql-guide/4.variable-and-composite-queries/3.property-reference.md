...
Starting from <ProductName /> 2.6.0, [Schema-related functions](../6.functions-and-expressions/4.schema.md) are supported. The preceding example can be rewritten as follows in <ProductName /> {{ nebula.release}} to produce the same results: 

```ngql
GO FROM "player100" OVER follow YIELD properties($^).name AS startName, properties($$).age AS endAge;
```

<ProductName /> {{ nebula.release}} is compatible with both new and old syntax.
