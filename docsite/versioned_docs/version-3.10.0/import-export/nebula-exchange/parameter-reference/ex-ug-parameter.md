...
You can use either a plaintext password or an RSA encrypted password when setting the password for connecting to <ProductName /> in the configuration file.
...
|`tags.partition`|int|`32`|Yes|The number of partitions to be created when the data is written to <ProductName />. If `tags.partition ≤ 1`, the number of partitions to be created in <ProductName /> is the same as that in the data source.|
|`tags.filter`|string|-|No|The filtering rule. The data that matches the filter rule is imported into <ProductName />. For information about filtering formats, see [Dataset](https://spark.apache.org/docs/latest/api/scala/org/apache/spark/sql/Dataset.html#filter(conditionExpr:String):org.apache.spark.sql.Dataset[T]).|
...
|`edges.partition`|int|`32`|Yes|The number of partitions to be created when the data is written to <ProductName />. If `edges.partition ≤ 1`, the number of partitions to be created in <ProductName /> is the same as that in the data source.|
|`edges.filter`|string|-|No|The filtering rule. The data that matches the filter rule is imported into <ProductName />. For information about filtering formats, see [Dataset](https://spark.apache.org/docs/latest/api/scala/org/apache/spark/sql/Dataset.html#filter(conditionExpr:String):org.apache.spark.sql.Dataset[T]).|
