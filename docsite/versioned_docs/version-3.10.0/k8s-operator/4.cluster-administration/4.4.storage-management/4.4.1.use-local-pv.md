...
=== "Use Local PV on GKE"
    ...
    4. In the <ProductName /> cluster configuration file, specify `spec.storaged.dataVolumeClaims` or `spec.metad.dataVolumeClaim`, and the StorageClass needs to be configured as `local-nvme`. For more information about cluster configurations, see [Create a <ProductName /> cluster](../4.1.installation/4.1.1.cluster-install.md). 

      ```yaml title="Partial configuration of the <ProductName /> cluster"
      ...
      metad: 
        dataVolumeClaim:
          resources:
      ...
      After the <ProductName /> is deployed, the Local PVs are automatically created.
